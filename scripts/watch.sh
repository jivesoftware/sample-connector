...
psql -U postgres -h localhost "sample-connector" -c "select count(*) from (select modtime, count(workerid) from activitylog group by workerid, ownerid,modtime having count(workerid) > 1) as duplicate;";
